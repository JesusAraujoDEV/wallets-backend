...
const { buildIncludedGroupWhere, firstOfMonth, daysInMonth, MONTH_NAMES } = require('./shared');
...
async function getComparativeMoMIncome({ userId, date, groupId }) {
  ...
  const cpName = `${MONTH_NAMES[currentStart.getUTCMonth()]} MTD (1-${currentEnd.getUTCDate()})`;
  const ppName = `${MONTH_NAMES[prevStart.getUTCMonth()]} MTD (1-${prevEndMTD.getUTCDate()})`;
...
module.exports = { getComparativeMoMIncome };
